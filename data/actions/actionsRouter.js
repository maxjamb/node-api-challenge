...
router.put(
  "/:id",
  validateActionId,
  validateActionsAtLeastOneBody,
  (req, res, next) => {
    const { description, notes, completed } = req.body;
    Actions.update(req.action.id, { description, notes, completed })
      .then(action => {
...
function validateActionsAtLeastOneBody(req, res, next) {
  ...
  if (!description && !notes && typeof completed === "undefined") {
    res.status(400).json({
      message:
        "Please provide one of these fields to change: description, notes or completed."
    });
  }
